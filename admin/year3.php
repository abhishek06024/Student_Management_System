  ...
  { 
    $count=1;
      while ($result=mysqli_fetch_assoc($data)) {
          ...
          <td>".$result['status']."</td>
          <td>".$result['sem']."</td>
          <td><a href = 'edit3.php?Roll_Number=$result[Board_Roll_No]&emil=$result[email]&mob=$result[mobile]&first=$result[first_name]&last=$result[last_name]&father=$result[father_name]&branch=$result[branch]&status=$result[status]&sem=$result[sem]'><input type='submit' value='Edit/Update' id='edit'></a></td>
         
          <td><a href = 'delete3.php?Roll_Number=$result[Board_Roll_No]' onclick='return checkdelete()'><input type='submit' value='Delete' id='del'></a></td>
          </tr>
          ";
          $count++;
      }
